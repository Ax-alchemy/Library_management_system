...
def createTableBook(conn):
    cur = conn.cursor()
    query = 'CREATE TABLE Book(Isbn VARCHAR(10) NOT NULL, Title VARCHAR(255), Cover VARCHAR(1000), Availability INT NOT NULL DEFAULT 1, CONSTRAINT BKPK PRIMARY KEY(Isbn));'
    cur.execute(query)
