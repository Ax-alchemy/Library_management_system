...
def index(request):
	...
	if(request.method == "POST"):
		if('search' in request.POST):
			...
			query = "SELECT BkAthr.Isbn, BkAthr.Title, BkAthr.authors, BkAthr.Cover, BkAthr.Availability FROM (SELECT Book.Isbn, Book.Title, GROUP_CONCAT(Authors.Name) authors, Book.Cover, Book.Availability FROM Book,Book_Authors,Authors WHERE Book.Isbn = Book_Authors.Isbn AND Book_Authors.Author_id = Authors.Author_id GROUP BY Book.Isbn) AS BkAthr WHERE "+comparision
